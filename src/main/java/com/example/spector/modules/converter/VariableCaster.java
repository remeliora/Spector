...
public class VariableCaster {
    ...
    public Object convert(Parameter parameter, Variable variable) {
        DataType dataType = parameter.getDataType();
        TypeCaster<?> typeCaster = typeCasterFactory.getTypeCaster(dataType);
        return castTo(parameter, dataType, variable, typeCaster);
    }

    private <T> T castTo(Parameter parameter, DataType dataType, Variable variable, TypeCaster<T> typeCaster) {
        if (variable == null) {
        ...
        return typeCaster.cast(parameter, variable);
    }
}
