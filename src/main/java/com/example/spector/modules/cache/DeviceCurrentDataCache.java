...
public class DeviceCurrentDataCache {
    ...
    public DeviceCurrentData updateCurrentData(Long deviceId, DeviceCurrentData data) {
        // System.out.println("Обновление кэша для устройства " + deviceId + " с данными: " + data);
        return data; // Spring Cache положит это значение в кэш
    }

    // Метод для получения актуальных данных устройства из кэша
    @Cacheable(value = "currentDeviceData", key = "#deviceId", unless = "#result == null") // Кэшируем, если результат не null
...
//        log.debug("Промах кэша для устройства {}", deviceId);
...
//        log.debug("Удаление из кэша данных для устройства {}", deviceId);
    }
}
